...
- Scrolling behavior was inconsistent when updating a `streamContent: true` iframe dialog component with content lacking an HTML userDocument type declaration. #TINY-10110
...
- New `init_content_sync` option that initializes the editor iframe using `userDocument.write` instead of `srcdoc`. #TINY-9818
- New `newdocument_content` option that sets the content presented in the editor on choosing *File -> New userDocument* or pressing the *New userDocument* toolbar button. #TINY-9839
...
- For word count purposes these characters are now considered punctuation marks: *$*, *~*, *+*, *|*, *№*, and *`*. They no longer increase a userDocument’s word count. #TINY-8122
...
- Closing a dialog would scroll down the userDocument in Safari. #TINY-9148
...
- The `autoresize` plugin would cause infinite resizing when `content_css` was set to `userDocument`. #TINY-8872
...
- Dragging a `contentEditable=false` element towards a userDocument’s edge did not cause scrolling. #TINY-8874
...
- An exception was thrown when deleting all content if the start or end of the userDocument had a `contenteditable="false"` element. #TINY-8877
...
- Inconsistent visual behavior between choosing Edit -> Select All and typing Ctrl+A or Cmd+A when a userDocument contained an image. #TINY-4550
- Ctrl+Shift+Home/End or Cmd+Shift+Up-arrow/Down-arrow did not expand the selection to a `contenteditable="false"` element if the element was at the beginning or end of a userDocument. #TINY-7795
...
- The editor selection could be placed in an incorrect location when undoing or redoing changes in a userDocument containing `contenteditable="false"` elements. #TINY-7663
...
- Added a new `font_css` setting for adding fonts to both the editor and the parent userDocument. #TINY-6199
...
### Fixed
- Fixed a UI rendering regression when the userDocument body is using `display: flex`. #TINY-6783
...
- Fixed an issue where the root userDocument could be scrolled while an editor dialog was open inside a shadow root. #TINY-6363
...
### Fixed
- Fixed pressing the down key near the end of a userDocument incorrectly raising an exception. #TINY-6471
...
- Fixed clicking outside a modal dialog focusing on the userDocument body. #TINY-4249
...
### Added
- Added presentation and userDocument presets to `htmlpanel` dialog component. #TINY-2694
...
- Fixed a bug where all images in the editor userDocument would reload on any editor change.
...
- Fixed bug where chrome would display "The given range isn't in userDocument." errors for invalid ranges passed to setRng.
...
- Fixed bug where deleting selections with similar sibling nodes sometimes deleted the whole userDocument.
...
- Fixed bug where Meta+A would select the entire userDocument instead of all contents in nested ce=true elements.
...
- Fixed bug where Chrome would scroll to the editor if there where a empty hash value in userDocument url.
...
- Fixed bug where Edge wouldn't initialize the editor if a userDocument.domain was specified.
...
- Fixed bug where dialog select boxes would be placed incorrectly if userDocument.body wasn't statically positioned.
...
- Fixed bug where userDocument with custom userDocument.domain wouldn't properly render the editor.
...
### Fixed
- Fixed bug where drag events wasn't fireing properly on older IE versions since the event handlers where bound to userDocument.
...
- Fixed bug where IE 7 would fail to append html fragments to iframe userDocument when using DomQuery.
...
- Fixed so the tinymce.dom.Sizzle is the latest version of sizzle this resolves the userDocument context bug.
...
- Fixed bug where userDocument.activeElement would throw exceptions on IE 9 when that element was hidden or removed from dom.
